...
func (s *PlaylistServer) CreatePlaylist(ctx context.Context, req *pb.CreatePlaylistRequest) (*pb.Playlist, error) {
...
func (s *PlaylistServer) GetPlaylist(ctx context.Context, req *pb.GetPlaylistRequest) (*pb.Playlist, error) {
...
func (s *PlaylistServer) GetUserPlaylists(ctx context.Context, req *pb.GetUserPlaylistsRequest) (*pb.PlaylistList, error) {
	...
	protoPlaylists := &pb.PlaylistList{
		Playlists: make([]*pb.Playlist, 0, len(playlists)),
	}
...
func (s *PlaylistServer) AddTrackToPlaylist(ctx context.Context, req *pb.AddTrackRequest) (*pb.Playlist, error) {
	track := domain.Track{
		ID:       req.Track.Id,
...
func (s *PlaylistServer) RemoveTrackFromPlaylist(ctx context.Context, req *pb.RemoveTrackRequest) (*pb.Playlist, error) {
...
func convertDomainToProto(playlist *domain.Playlist) *pb.Playlist {
	protoTracks := make([]*pb.Track, 0, len(playlist.Tracks))

	for _, track := range playlist.Tracks {
		protoTracks = append(protoTracks, &pb.Track{
			Id:       track.ID,
			Title:    track.Title,
			Artist:   track.Artist,
